...
int handle_fp_query(char *fp_filename) {
	...
	int n = strlen(fp_filename);
	memcpy(fp, fp_filename, FP_LEN);
	memcpy(filename, &fp_filename[FP_LEN], n - FP_LEN);
	filename[n - FP_LEN] = '\0';
	// printf("----File FP: %s, File name: %s**\n", fp, filename);

	// look up this fp for dedu in server
...
void recv_chunk(int sockfd){
	...
	while(1){
		...
		if(header->type ==  FILE_FIRST_PACKET){ // file first pakcet (fp+file name), Definitely new File
			rlen = Readn(sockfd, buf, header->len);
			memcpy(fp, buf, FP_LEN);
			memcpy(filename, buf + FP_LEN, rlen - FP_LEN);
			...
			rlen = Readn(sockfd, buf, header->len);
			// printf("4. fp query: %s\n", buf);
			int exists = handle_fp_query(buf);
			char *msg = "new";
			if(exists)
				msg = "duplicated";
